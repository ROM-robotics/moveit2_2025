...
public:
    Controller()  : Node("commander_from_ui")
    {
        ...
        // Changed to std::vector<std::string> based on previous error
        this->declare_parameter<std::vector<std::string>>("planning_pipelines", std::vector<std::string>());
        
        subscription_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            "/target_point", 10, std::bind(&Controller::listener_callback, this, std::placeholders::_1));
        
        // Example of pink info message
        RCLCPP_INFO(this->get_logger(), "\033[1;35m Subscribed to /target_point topic.\033[0m");
    ...
    void initializeMoveIt()
    {
        ...
        // Example of pink info message
        RCLCPP_INFO(this->get_logger(), "\033[1;35m MoveItCpp instance created successfully.\033[0m");
...
private:
    ...
    {
        if (!ur5_arm_) {
            RCLCPP_ERROR(this->get_logger(), "UR5 arm planning component not initialized. Skipping move_to.");
            return;
        }
    ...
    {
        if (!ur5_hand_) {
            RCLCPP_ERROR(this->get_logger(), "Robotiq gripper planning component not initialized. Skipping gripper_action.");
            return;
        }
...
int main(int argc, char* argv[])
{
    ...
    // Call the new initialization method *after* the node is created as a shared_ptr
    controller_node->initializeMoveIt();
